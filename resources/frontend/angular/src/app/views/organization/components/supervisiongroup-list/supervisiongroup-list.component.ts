...
@Component({
  selector: 'app-supervisiongroup-list',
  templateUrl: './supervisiongroup-list.component.html',
  styleUrls: ['./supervisiongroup-list.component.css'],
})
export class SupervisiongroupListComponent implements OnInit {
  supervisiongroups!: SupervisionGroupCollection;
  pending: boolean = false;
  display!: DisplayResourceCollection;
  search!: SupervisionGroupSearch;

  private searchSubject: Subject<SupervisionGroupSearch> = new Subject();
  ...
  ngOnInit(): void {
    ...
    this.store.select('supervisiongroups').subscribe((supervisiongroups) => {
      ...
      this.display = supervisiongroups.display;
    });

    if (this.supervisiongroups.meta === null) this.dispatchLoad();
  }

  open(id: number) {
    this.router.navigate([
      `/management/supervisiongroups/supervisiongroup/${id}`,
    ]);
  }
  ...
  doSearch(from: string, event: any) {
    ...
    if (
      this.search[from as keyof SupervisionGroupSearch] === event.target.value
    )
      return;

    this.search = {
      ...this.search,
      [from as keyof SupervisionGroupSearch]: event.target.value,
    };
  ...
  toKey(sb: string): keyof SupervisionGroupSearch {
    return sb as keyof SupervisionGroupSearch;
  }
}
