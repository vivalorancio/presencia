...
export class AreaListComponent implements OnInit {
  ...
  private searchSubject: Subject<AreaSearch> = new Subject();
  ...
  ngOnInit(): void {
    ...
    this.store.select('areas').subscribe((areas) => {
      ...
      this.display = areas.display;
    });

    if (this.areas.meta === null) this.dispatchLoad();
  }
  ...
  doSearch(from: string, event: any) {
    ...
    if (this.search[from as keyof AreaSearch] === event.target.value) return;

    this.search = {
      ...this.search,
      [from as keyof AreaSearch]: event.target.value,
    };
